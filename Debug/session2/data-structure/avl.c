...
const student_t *generate_student() {
...
/* Insert a new node. */
void avl_insert(avl_tree_t *tree, int key, const void *value) {
    avl_node_t *node = NULL;
...
void avl_traverse_node_dfs(avl_node_t *node, int depth) {
    ...
    printf("%d:%p %d\n", node->key, node->value, avl_balance_factor(node));
...
int main(int argc, char **argv) {
    ...
    for (size_t i = 0; i < 7; i++) {
        avl_insert(tree, i, generate_student());
    }
    return 0;
}
